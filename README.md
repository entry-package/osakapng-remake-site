...
Strikinglyの公開ページ109件から、記事・プロフィール101件を元のURLで再構築した静的サイトです。本文・画像を保全し、記事内リンクの修正は別表で管理しています。現在の公開プロフィールから不足情報を補っています。2026-09-14に確認済み修正版を既存のGitHub Pagesへ公開しました。独自ドメインのDNS切替は管理先の本人ログイン待ち、Strikingly解約は未実施です。公開記録は `content/publication-20260914.json` を参照。
...
- GitHub Pages公開・HTTPS・全113ルートの読戻しは完了。独自ドメインのDNS・HTTPS・www/apex・メール用DNSの確認は、ムームードメインの本人ログイン後に進める。
