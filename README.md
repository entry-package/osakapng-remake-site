...
- GitHub Pagesの初回URLでは公開・HTTPS・全113ルートの読戻しが完了。独自ドメインのDNS変更とメール・既存サブドメインの維持は43項目PASS。GitHub側の証明書発行とEnforce HTTPSを確認済みで、正式HTTPSから398件の内容一致と13件の証明書・転送検証がPASS。Chromeの正式HTTPS URLで新サイト表示、タコペン画像の読み込み、横はみ出しがないことを確認済み。
